...
def request_item_list():
    ...
    add_request_data("Item_Request", "List_of_Devices",
                     "1", "NULL", time.time())
...
def request_item_options():
    ...
    print(items)
    add_request_data("Item_Request", "List_of_Item_Options", "1",
                     "NULL", time.time())
    return json.dumps(items)
...
def update_item_state():
    ...
    if obj == "items":
        ...
        if e.enforce(sub, obj, act):
            ...
            print(val)
            print(str(opt).upper())
            r = requests.post("http://localhost:8080/rest/items/" +
                              str(val), data=str(opt).upper())
            print(r.text)
            add_request_data("Item_Request", "Update_Item_State",
                             "1", "NULL", time.time())
            print("Item Option Updated")
            return "1"
        else:
            add_request_data("Item_Request", "Update_Item_State",
                             "4", "Insufficient_Access_Credentials",
                             time.time())
...
def request_policies_with_item_access():
    add_request_data("Policy_Request", "Get_Valid_Policies_For_Items",
                     "1", "NULL", time.time())
...
def request_policies1():
    add_request_data("Policy_Request", "Get_All_Policies", "1",
                     "NULL", time.time())
...
def update_policies():
    ...
    try:
        ...
        add_request_data("Policy_Request", "Update_Policies",
                         "2", "Not_Correctly_Encrypted", time.time())
        print("ERROR: Request was not parsed from JSON")
        return "2"
    try:
        sub = req["subject"]
        ...
        add_request_data("Policy_Request", "Update_Policies",
                         "3", "Lacking_Policy_Credentials", time.time())
        print("ERROR: Policy Credentials not found")
        return "3"
        ...
        if e.enforce(sub, obj, act):
            ...
            if act == "add":
                ...
                if PolicyMethods.find_row(new_string):
                    ...
                    add_request_data("Policy_Request", "Add_Policy",
                                     "Unsure", "Policy_Already_Exists",
                                     time.time())
                    return "5"
                else:
                    print("New Policy Added")
                    PolicyMethods.add_policy(new_string)
                    add_request_data("Policy_Request", "Add_Policy", "1",
                                     "NULL", time.time())
                    return "1"
            elif act == "edit":
                print("Edit Action")
                ...
                if PolicyMethods.find_row(old_policy):
                    print("Found row")
                    add_request_data("Policy_Request", "Edit_Policy", "1",
                                     "NULL", time.time())
                    PolicyMethods.edit_policy(old_policy, new_policy)
                    return "1"
                else:
                    print("Did not find row")
                    add_request_data("Policy_Request", "Edit_Policy", "Unsure",
                                     "Policy_Does_Not_Exist", time.time())
                    resp = "The policy requested doesnt exist"
                    return "5"
            elif act == "del":
                print("Delete Action")
                policy_to_delete = "p, " + req["old"]
                if PolicyMethods.find_row(policy_to_delete):
                    add_request_data("Policy_Request", "Delete_Policy", "1",
                                     "NULL", time.time())
                    PolicyMethods.delete_policy(policy_to_delete)
                    return "1"
                else:
                    add_request_data("Policy_Request", "Delete_Policy",
                                     "Unsure", "Policy_Does_Not_Exist",
                                     time.time())
                    return "5"

        else:
            # deny the request, show an error
            add_request_data("Policy_Request", "Update_Policies", "4",
...
def request_analytics():
    try:
        ...
        add_request_data("Analytics_Request", "Request_Analytics",
                         "2", "Not_Correctly_Encrypted", time.time())
        print("ERROR: Request was not parsed from JSON")
        return "2"
    try:
    ...
    if obj == "analytics":
        ...
        if e.enforce(sub, obj, act):
            ...
            if choice == 1:
                ...
                add_request_data("Analytics_Request", "Request_Analytics",
                                 "1", "NULL", time.time())
                return json.dumps(AnalyticMethods.
                                  get_successful_request_data())
            elif choice == 3:
                add_request_data("Analytics_Request", "Request_Analytics",
                                 "1", "NULL", time.time())
                return json.dumps(AnalyticMethods.
                                  get_unsuccessful_request_data())
            else:
                add_request_data("Analytics_Request", "Request_Analytics",
                                 "5", "Request_Type_Unavailable",
                                 time.time())
                return 5
...
def receive_feedback():
    try:
        ...
        add_request_data("Feedback_Request", "Receive_Feedback", "2",
                         "Not_Correctly_Encrypted", time.time())
        print("ERROR: Request was not parsed from JSON")
        return "2"
    try:
        ...
        add_request_data("Feedback_Request", "Receive_Feedback", "3",
                         "Lacking_Policy_Credentials", time.time())
        print("ERROR: Policy Credentials not found")
        return "3"
    ...
    add_request_data("Feedback_Request", "Receive_Feedback", "1",
                     "NULL", time.time())
    return "1"


def add_request_data(request_type, resource_requested,
                     response, reason, timestamp):
    with open('RequestData.csv', 'a') as csv_file:
        csv_file.writelines(request_type + "," + resource_requested +
                            "," + response + "," + reason + "," +
                            str(timestamp) + "\n")
